...
namespace NSpec.Domain.Formatters
{
    ...
    public class ConsoleFormatter : IFormatter
    {
        ...
        public ConsoleFormatter(ITestListener listener)
        {
        ...
        {
            contexts.Do(c => WriteLine(Write(c)));

            WriteLine(FailureSummary(contexts));

            WriteLine(Summary(contexts));
        }

        private void WriteLine(string line)
        {
            if (listener != null)
                listener.WriteLine(line, new Category());
            else
